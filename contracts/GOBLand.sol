...
contract GOBLand is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    modifier onlyAuthorizedContracts() {
        require(
            ...
            "Only allowed from authorized contracts"
        );
        _;
    }
        ...
        whenNotPaused
    {
        ...
        require(
            _verify(_leaf(msg.sender, amount), proof),
    ...
    function externalMint(address addr, uint256 amount)
        public
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % maxLands;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)

        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % maxLands;
        }
